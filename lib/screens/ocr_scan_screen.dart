...
class _OCRScanScreenState extends State<OCRScanScreen> {
  ...
  Future<void> _scanFromGallery() async {
    ...
    try {
      ...
      if ((result['medicines'] as List).isNotEmpty) {
        _saveToGlobalContext(result); // ✅ 챗봇 컨텍스트 저장
  ...
  void _saveToGlobalContext(Map<String, dynamic> result) {
    ...
    print('챗봇 컨텍스트 저장: ${GlobalMedicineList.lastScannedMedicines}');
  }
            ...
            children: [
              ...
              Container(
                ...
                child: Row(children: [
                  ...
                  Expanded(
                    child: Text(
                      '💬 대화하기 탭에서 이 약에 대해 질문해보세요!\n예) "이 약 부작용이 뭐야?", "밥 먹고 먹어야 해?"',
              ...
              if (verifiedMedicines.isNotEmpty) ...[
                Row(children: [
                  const Text('✅ 식약처 DB 확인된 약품',
                      style: TextStyle(fontWeight: FontWeight.bold, fontSize: 13)),
                  const SizedBox(width: 6),
                  Container(
                    padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 2),
                    decoration: BoxDecoration(color: Colors.green, borderRadius: BorderRadius.circular(10)),
                    child: Text('${verifiedMedicines.length}개',
                ...
                ...verifiedMedicines.map((med) => Padding(
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                      color: (med['db_matched'] == true) ? Colors.blue.shade50 : Colors.grey.shade50,
                      borderRadius: BorderRadius.circular(8),
                      border: Border.all(
                          color: (med['db_matched'] == true) ? Colors.blue.shade100 : Colors.grey.shade300),
                    ),
                    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Row(children: [
                        Expanded(child: Text(med['name'] ?? '',
                            style: TextStyle(fontWeight: FontWeight.bold, fontSize: 13,
                                color: (med['db_matched'] == true) ? Colors.blue : Colors.grey.shade700))),
              ...
              Text('📋 인식된 약품 (${medicines.length}개)',
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('약 봉투 스캔'), backgroundColor: Colors.green),
      body: Center(
        child: _isLoading
            ? Column(mainAxisAlignment: MainAxisAlignment.center, children: [
          const CircularProgressIndicator(color: Colors.green),
          const SizedBox(height: 20),
          Text(_loadingMessage, style: const TextStyle(fontSize: 16, color: Colors.green)),
          const SizedBox(height: 8),
          const Text('AI가 약품명을 분석 중이에요...', style: TextStyle(fontSize: 13, color: Colors.grey)),
        ])
            : Padding(
